...
class ContactsController < ApplicationController
	...
	process_line = ""
        if params["export"]
			...
			next if line =~ /^#/
			process_line = line.encode('UTF-8', 'binary', invalid: :replace, undef: :replace, replace: '')
                  Contact.import(@current_user,process_line)
                end
            rescue ActiveRecord::RecordInvalid => e
                flash[:error] = {:title => e.to_s,:info => e.record.inspect + e.record.errors.inspect}
			rescue => e
                flash[:error] = {:title => e.to_s,:info => process_line}
            else
